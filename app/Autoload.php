...
class Autoload {
    ...
    protected function handleRequest($type = null, $forcedMethod = null) {
        ...
        // Tambahkan pengecekan untuk init request
        if ($pathParts[0] === 'init') {
            return $this->handleInitRequest($pathParts);
        }
        
        // Ubah pengecekan untuk worker - cek apakah path mengandung 'worker'
        if (strpos($path, 'worker') !== false) {
            $tatiyeNet = new NgoreiEngine();
        ...
        try {
            ...
            if ($pathParts[0] =='sdk' && $pathParts[0] =='api') {
    ...
    protected function handleCssJsRequest($path) {
        // Daftar lokasi yang mungkin untuk file CSS/JS
        $possibleLocations = [
            PUBLIC_DIR . $path,
        ...
        foreach ($possibleLocations as $location) {
            if (file_exists($location)) {
                return $this->serveStaticFile($location);
            }
        }
    ...
    protected function serveStaticFile($path) {
        ...
        // Implementasi ETag untuk caching
        $etag = '"' . md5_file($path) . '"';
        header('ETag: ' . $etag);
    ...
    protected function template($filePath) {
        ...
        if (!empty(MOBILE_DETECT)) {
            ...
            switch($deviceType) {
                case 'mobile':
                    $templateFile = ROOT_PUBLIC."/mobile.html";
                    break;
                case 'tablet':
                    $templateFile = ROOT_PUBLIC."/tablet.html";
                    break;
                default:
    ...
    protected function handleImageRequest($path) {
        // Normalize path terlebih dahulu
        $path = $this->normalizeImagePath($path);
        
        // Debug log
        error_log("[Image Request] Original path: " . $path);
        
        // Hapus 'img/' dari awal path
        $imagePath = preg_replace('/^img\//', '', $path);
        error_log("[Image Request] Cleaned path: " . $imagePath);
        
        // Daftar lokasi yang mungkin untuk mencari gambar
        $possibleLocations = [
            PUBLIC_DIR . 'img/' . $imagePath,
            PUBLIC_DIR . 'images/' . $imagePath, // Tambahkan alternatif folder
            ASSET . 'img/' . $imagePath,
            ASSET . 'images/' . $imagePath, // Tambahkan alternatif folder
            BASEPATH . '/uploads/img/' . $imagePath,
            BASEPATH . '/assets/img/' . $imagePath,
            BASEPATH . '/assets/images/' . $imagePath, // Tambahkan alternatif folder
            // Tambahkan path langsung jika path sudah lengkap
            $imagePath,
            PUBLIC_DIR . $imagePath,
            ASSET . $imagePath
        ];
        
        // Log semua kemungkinan lokasi
        error_log("[Image Request] Checking locations:");
        foreach ($possibleLocations as $location) {
            error_log("Checking: " . $location . " - Exists: " . (file_exists($location) ? 'Yes' : 'No'));
            if (file_exists($location)) {
                error_log("Found file at: " . $location);
                if ($this->isValidImage($location)) {
                    error_log("[Image Request] Valid image found at: " . $location);
                    return $this->serveImage($location);
                } else {
                    error_log("[Image Request] File exists but not a valid image: " . $location);
                }
            }
        }
        
        error_log("[Image Request] Image not found in any location. Path: " . $path);
    ...
    protected function serveImage($path) {
        ...
        // Implementasi partial content/range requests
        if (isset($_SERVER['HTTP_RANGE'])) {
            $this->handleRangeRequest($path);
            exit;
        }
        
        // Stream file dengan buffer kecil untuk efisiensi memori
        if ($fp = fopen($path, 'rb')) {
    ...
    protected function handleRangeRequest($path) {
        $size = filesize($path);
        $length = $size;
        $start = 0;
        $end = $size - 1;
        ...
        while (!feof($fp) && $sent < $length) {
            $buffer = fread($fp, min(8192, $length - $sent));
            echo $buffer;
            flush();
            $sent += strlen($buffer);
        }
    ...
    // Tambahkan method baru untuk menangani init request
    protected function handleInitRequest($pathParts) {
        // Hapus 'init' dari path
        array_shift($pathParts);
        
        // Ambil service name (handler dalam kasus ini)
        $service = $pathParts[0] ?? '';
        
        // Validasi method yang digunakan (hanya terima POST)
        $method = strtolower($_SERVER['REQUEST_METHOD']);
        if ($method !== 'post') {
            return $this->sendJsonError('Init handler tidak mendukung method POST', 405);
        }
        ...
        try {
            // Bangun path ke file handler di folder system/alias
            $handlerPath = BASEPATH . '/app/system/alias/' . $service . '.php';
            
            error_log("Mencoba mengakses init handler di: " . $handlerPath);
            
            if (!file_exists($handlerPath)) {
                throw new \Exception("File handler tidak ditemukan: $service");
            }
            ...
            error_log("Error dalam handleInitRequest: " . $e->getMessage());
            return $this->sendJsonError($e->getMessage());
        }
    }
}
