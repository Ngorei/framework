...
class tatiye {
    ...
    public static function setPDO($tabel, $bin='*', $where='', $limit='LIMIT 1') {
        try {
        ...
        foreach($files as $net => $file) {
            ...
            if (is_file($full_path)) {
                ...
                if (count($pathparts) > 1) {
                    ...
                    $result['api'][self::newKey(str_replace("/", "_", $pathpreg))] = $str === 'true' ? false : false;
                }
            }
            if (is_dir($full_path)) {
         ...
         if (APP_ENV=='development') {
               if (!file_exists($file)) {
                ...
                $jsonData = file_get_contents($file);
                   ...
                   $data = json_decode($jsonData, true); 
                   $errors = NgoreiLogger::detectAssetsErrors($jsonData);

                if (json_last_error() !== JSON_ERROR_NONE) {
                   if (!empty($errors)) {
                       NgoreiLogger::showErrorAlert($errors[0]);
                   } 
                   return $errors;
  ...
  */
  ...
  /* and class title */
    ...
    /* and class pathRequest */
        ...
        */
        public static function findByKey($key){
                $pathPayload = self::pathPayload();
                 foreach ($pathPayload['sdk'] as $index => $value) {
                     if ($index === $key) {
                         return array(
                             'key'    => $index,
                             'package'=> $value
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Initializes writeLog 
    |--------------------------------------------------------------------------
    | Develover Tatiye.Net 2022
    | @Date  
    */
    public static function writeLog($message){
    $logDir = 'D:/Ngorei/app/logs';
    $logFile = $logDir . '/upload1_' . date('Y-m-d') . '.log';
