...
class Ngorei {
  ...
  {
      $actualFile = substr($file, 7);
      if (empty(trim($actualFile))) {
          return;
      }

      $url = $this->buildAssetUrl($actualFile);
      $attrs = $this->buildAttributes(array_merge($attributes, ['type' => 'module']));

      // Tambahkan indentasi 4 spasi
      $indent = "    ";
      
      $this->assets[$position][] = sprintf('%s<script%s src="%s"></script>',
          $indent,
          $attrs,
  ...
  private function processRegularAsset(string $file, string $position, array $attributes): void
  {
      ...
      switch ($ext) {
          case 'css':
              ...
              $this->assets[$position][] = sprintf('%s<link%s href="%s">',
                  $indent,
                  $attrs,
                  htmlspecialchars($url, ENT_QUOTES, 'UTF-8')
              );
