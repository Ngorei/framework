...
class NgoreiEngine {
  ...
  // Tambahkan property untuk frontend
  private $frontend;
  private $parser;

  // Tambahkan setelah property di bagian atas class
  private $codeHighlightOptions = [
      'maxHeight' => '300px',
   ...
   */
  private function domIconElement(string &$content): void 
  {
    $content = \app\element\Icon::transform($content);
  }

  /**
   * Fungsi baru untuk mendapatkan format asli icon
   */
  public function getOriginalIconFormat(string $iconType, array $attrs): string 
  {
      return sprintf('<%s name="%s" color="%s" size="%s" />', 
          $iconType,
          $attrs['name'] ?? '',
          $attrs['color'] ?? '',
          $attrs['size'] ?? ''
  ...
  public function DomHTML(string &$content, bool $removeVars = true, bool $return = true, bool $compress = false): string
  {
    try {
        ...
        // Tambahkan pemanggilan domEscodeElement sebelum parsing lainnya
        $this->domEscodeElement($content);
        // Proses semua komponen UI melalui NexaUI
        $content = \app\element\NexaUI::transform($content);
        ...
        error_log("Error in DomHTML: " . $e->getMessage());
  ...
  public function Worker(string $varfile, bool $removeVars = false, bool $return = true, bool $compress = false): string
  {
    ...
    $footerContent = '';
    $asset = array_merge(tatiye::assets('footer'),tatiye::assets('libraries'));
    $scripts = '';
    ob_start();
     $footerContent = "\n" . PHP_EOL;
  ...
  {
      return \app\element\Utility::transform($content);
  }

  private function domTextInput(string &$content): void 
  {
    // Pastikan konten diubah dengan hasil transformasi
    $content = \app\element\TextInput::transform($content);
  }

  private function domSelectionInput(string &$content): void 
  {
    // Pastikan konten diubah dengan hasil transformasi
    $content = \app\element\SelectionInput::transform($content);
  }
  
  private function domFileInput(string &$content): void 
  {
    // Pastikan konten diubah dengan hasil transformasi
    $content = \app\element\FileInput::transform($content);
  }
  ...
  // Tambahkan di bagian property class
  private $escodeProcessor;

  // Tambahkan setelah property di bagian atas class
  private function initEscode(): void
  {
      if (!isset($this->escodeProcessor)) {
          $this->escodeProcessor = new \app\element\NgoreiEscode();
      }
  }

  // Tambahkan method baru untuk memproses tag Escode
  private function domEscodeElement(string &$content): void 
  {
      try {
          // Inisialisasi Escode processor
          $this->initEscode();
          
          // Transformasi konten menggunakan Escode
          $content = \app\element\NgoreiEscode::converCode($content);
          
      } catch (Exception $e) {
          error_log("Error processing Escode: " . $e->getMessage());
          // Dalam mode development, tampilkan error
          if (self::DEVELOPMENT_MODE) {
              $content = str_replace(
                  '<Escode',
                  '<!-- Escode Error: ' . htmlspecialchars($e->getMessage()) . ' --><Escode',
                  $content
              );
          }
      }
