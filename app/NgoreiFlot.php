...
class NgoreiFlot {
    ...
    public function toBarChart($result, $labelColumn, $valueColumn) {
        try {
            ...
            if (!isset($firstRow[$labelColumn]) || !isset($firstRow[$valueColumn])) {
                return [
                    'error' => 'Kolom tidak valid',
                    'message' => "Kolom $labelColumn atau $valueColumn tidak ditemukan",
                    'available_columns' => array_keys($firstRow)
                ];
            }
            ...
            foreach ($result as $index => $row) {
                $data[] = [
                    $index, 
                    is_numeric($row[$valueColumn]) ? (float)$row[$valueColumn] : 0
                ];
                $ticks[] = [
                    $index, 
            ...
            return [
                'error' => 'Error saat memproses data',
                'message' => $e->getMessage()
            ];
        }
    }
