...
export function Filebrowser(config) {
   ...
   var dataset = Tds.storageKey(config.storage, config.key);
                                ...
                                if (allowedFile) {
                                    ...
                                    if (fileName =='img') {
                                          ...
                                          reader.onload = function() {
                                              ...
                                              $("#label_thumbnail").html('Unggahan file sesuai ekstensi yang ditentukan ');
                                          ...
                                          reader.readAsDataURL(file); // Membaca file sebagai URL data
                                              ...
                                              $("#previewthumbnail").attr("src", basisData.baseURL+'assets/images/doc/'+fileName+".png");
                                              $("#label_thumbnail").html('Unggahan file sesuai ekstensi yang ditentukan ');
                                              $("#alert").html('Informasi Ambil foto'); 
                                         }
                                    ...
                                    $("#thumbnail").html('Tipe file tidak diperbolehkan. Silakan unggah file dengan tipe: ' + allowedTypes.join(', '));
                                }
                            } else {
    ...
    form.send = function() {
        const formData = validasiSend(fieldIDs, form, config.cradensial, config.status, "info_" + config.elementById, config,dataset);
        if (formData && typeof config.sendCallback === 'function') {
            ...
            .then(result => {
                if (result.status === 'success') {
                           $("#previewthumbnail").attr("src",window.Ngorei.baseURL+"img/Drive.png");
                           // Kosongkan input, select, dan textarea
                           $('input[type="file"], input[type="text"], input[type="email"], input[type="password"], input[type="number"], input[type="url"], input[type="tel"], input[type="date"], select, textarea').val('');
                           // Hapus tanda cek dari radio dan checkbox
                           $('input[type="radio"], input[type="checkbox"]').prop('checked', false);
                           if (config.quill.elementById) {
                             $('#area'+config.quill.elementById+" .ql-editor").html(' ');
                            }
                } else {
                      $("#label_thumbnail").html(result.message);
                }
                config.sendCallback(result); // Panggil sendCallback dengan hasil
            })
            .catch(error => {
                config.sendCallback({ status: 'error', message: 'Terjadi kesalahan saat mengirim data.' });
            });
        }
    };
}
...
export function validasiSend(fieldIDs, form, cradensial='', status='', idInfo='', config,dataset) {
...
fieldIDs.forEach(key => {
    ...
    if (!input.value.trim()) {
        ...
        formData['explorer']=getCookie('explorer')?getCookie('explorer'):false;
