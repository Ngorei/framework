...
export function presentation(attr) {
 ...
 const STG=new Dom.Storage()
        ...
        const slides = Array.from(document.querySelectorAll('#presentationID .bg-default')); // Perbarui pemilihan slide
         const slideshowID = document.querySelectorAll('#presentationID slide[key]');
         console.log("Semua elemen slide dengan atribut key:", slideshowID);

      if (slideshowID) {
           $.each(slideshowID,function(red, row){
              console.log(row)
               var elementById=$(row).attr('key');
               $(row).attr('id',elementById);
               slideshow(elementById)
