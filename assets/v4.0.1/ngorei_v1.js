...
export function jsElemenImport(base='') {
    //Tambahkan idi di file imfor module| export function initModule(file) {}
    const scriptElements = document.querySelectorAll("script[import]");
    if (scriptElements) {
       const hanst = window.location.hash.split("#");
      // console.log(hanst,scriptElements[0])
       const Path = window.Ngorei.baseURL + "public/" + hanst[1].split("/")[0] + "/";
       // const Path =hanst[1];
      if (scriptElements.length > 0) {
        scriptElements.forEach((script, index) => {
          const importPath = Path + script.getAttribute("import");
           ...
           import(importPath)
             .then((module) => {
               // Memanggil fungsi initModule dari modul yang diimpor
