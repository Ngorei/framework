...
(function(root, factory) {
    ...
    function createNavigasi(row) {
        // Dapatkan elemen target menu
        const menuTarget = document.getElementById(row.elementById);
        menuTarget.innerHTML = `

         ${Object.entries(row.navItem).map(([key, value]) => `
                 <a id="${key}"  class="${row.class.a}" onclick="mapMenu(['${value}','${key}']);" href="javascript:void(0);">${value[2] ? `<i class="${value[2]} mr-3px"></i>` : ''} ${value[0]}</a>
         `).join('')}
