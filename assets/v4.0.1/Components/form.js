...
export function validasiSend(fieldIDs, form, cradensial='', status='', idInfo='', config) {
  ...
  fieldIDs.forEach(key => {
    ...
    if (isValid) {
        ...
        let dataToStore = [formData]; // Simpan sebagai array dengan satu item
        localStorage.setItem(Tabel, JSON.stringify(dataToStore));
    ...
    var Tabel = "FROM_"+cradensial;
      if (navigator.onLine) {
          new Dom.Storage().From({
              ...
              "data": [formData], // Kirim hanya data terbaru
              "idInfo": idInfo,
              "config": config,
          });
