            ...
            if (formData) {
                if (typeof config.sendCallback === 'function') {
                    ...
                    try {
                        ...
                        console.log('sendCallback selesai');
                        resolve(formData);
                    } catch (error) {
                        console.error('Error dalam sendCallback:', error);
                        reject(error);
                    }
                } else {
                    console.log('sendCallback tidak ditemukan atau bukan fungsi');
                    resolve(formData); // Tetap resolve jika tidak ada callback
                }
            } else {
                console.log('Data formulir tidak valid');
...
export function validasiSend(fieldIDs, form, cradensial='', status='', idInfo='', config) {
  ...
  fieldIDs.forEach(key => {
    ...
    const inputType = input ? input.getAttribute('type') : null;
    ...
    if (!input.value.trim()) {
        ...
        if (!isValidURL(input.value.trim())) {
            isValid = false;
            infoSpan.textContent = `${label} harus berupa URL yang valid`;
        }
    } else {
